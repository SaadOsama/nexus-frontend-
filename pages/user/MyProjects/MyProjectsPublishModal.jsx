...
const PublishProjectModal = ({ isOpen, onClose, onProjectPublished }) => {
  // 🟢 FIX: Redux me user "user" slice ke andar store hota hai (userSlices.js:
  // userSlice.name = 'user', loginSuccess -> state.user = user), "auth" slice
  // nahi. Pehle "state.auth" padha ja raha tha jo hamesha undefined tha, is
  // liye user_id hamesha fallback "1" par chala jata tha — chahe koi bhi
  // login ho. Isi wajah se har naya publish hone wala project "Default User"
  // (id 1) ke naam par create ho raha tha.
  ...
  const handleSubmit = async (e) => {
    ...
    // 🟢 FIX: "|| 1" fallback hata diya. Agar kabhi user na mile (e.g. token
    // expire ho gaya ya user logged out state me hai), to project ko galat
    // user (id 1) ke naam par silently create karne ke bajaye request hi
    // rok do — taake ye bug dobara na ho.
    if (!user?.id) {
      console.error("No logged-in user found — cannot publish project.");
      return;
    }
